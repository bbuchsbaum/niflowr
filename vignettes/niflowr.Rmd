...
vignette: |
  %\VignetteIndexEntry{Getting started with niflowr} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
Profiles may also set Docker-only `platform` and `entrypoint` fields (for
example `platform: "linux/amd64"` under Rosetta, or `entrypoint: ""` to clear a
wrapping image ENTRYPOINT). `ni_run(..., timeout = ...)` or `runtime.timeout` in
`niflowr.yml` kills hung host/container processes.
...
lockfile consistency, and (when Docker images are local) that a profile's
payload command actually runs under its `entrypoint`/`platform`:
