...
Directories containing input and output files are automatically
bind-mounted:
...
result <- ni_fsl_bet(
  ...
  .container = list(type = "docker", image = "brainlife/fsl:6.0")
)
# Runs: docker run --rm -v /data:/data brainlife/fsl:6.0 bet ...
```

You can also set container defaults at the spec level in the JSON.
