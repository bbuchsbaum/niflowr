...
- `entrypoint`: overwrite or clear the image `ENTRYPOINT` (use `""` or `[]`
  when a shell entrypoint would silently ignore the niflowr command).
- `platform`: e.g. `"linux/amd64"` for amd64-only images on Apple Silicon.
...
lockfile consistency, and (for local Docker images) whether the payload command
actually executes:
...
Registration outputs carry explicit transform metadata: affine versus warp or
composite, the on-disk convention, and the source and target image inputs.
This lets niflowr load them as neurotransform morphisms without guessing from
an ambiguous `.mat` suffix:
...
`vignette("extending-niflowr")` vignette walks through the full loop
(spec → lint → wrapper → golden → test) using SynthStrip as a worked
example. The short version: create a JSON file following the schema at
`inst/schema/niflowr-spec-0.1.0.json`:
